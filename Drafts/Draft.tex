...
We are varying a parameter, $t$, in the exponent of the norm definition. So it is reasonable to expect that the behavior of the magnitude as $t$ varies will be logarithmic. If we plot $\magn_t(M_2)$ against $t$, we can see that it behaves just as we expect (see Figure \ref{fig:2pt_mag}).
\begin{figure}[H]
\[
...
\end{align*}
Unfortunately, this approach often does not work. The problem is that we are interested in considering spaces and scales such that some groups of points form clusters, meaning that they are very near each other. This will lead to $\zeta_A$ being very nearly not invertible, which has the effect of causing $\zeta_A^{-1}$ to have very large positive and negative values. These might mostly cancel to give us the magnitude we expect, but when we take the $L_1$ norm, the values no longer cancel. This causes our error bounds to get quite large when the points are not all separated (see Figure \ref{fig:failed_bound}).

\begin{figure}[h]
\[
\makebox[\textwidth][c]{\includegraphics[width = 0.9\paperwidth]{../Figures/error_1_correct.png}}
\]
\caption{A plot of the magnitude of a four-point square space, approximated by the product of two identical two-point spaces, with error bars calculated using the above technique when possible. Until all four points separate, the bounds are not useful or the limit diverges.}
...
This formula gives us a way of bounding how different the magnitude of the Euclidean spaces is from the product space. We can numerically calculate values for $\magn_t(M), \magn_t(A \times B)$, and the error term, and plot them.
