...
|\Omega/G| = \lambda f^T.
\]
In the next example, we will walk through all the steps involved in counting the orbits of a simple action.
...
\]
\caption{A plot of $\chi_t(M_2)$ against $t$ on a logarithmic scale.}
\label{fig:2pt_mag}
\end{figure}
\end{examp}
...
\caption{$A$ and $B$}
\end{figure}
Suppose that $A(a_1, a_2) = 1, B(b_1, b_2) = 2$. Recall that the monoidal product is $+$. So the tensor product category $A \times B$ looks like this:
\begin{figure}[H]
\[
\includegraphics[width=.4\paperwidth]{../Figures/gen_met_small_prod}
\]
\caption{$A \times B$. It is impossible to draw this product space to scale because of the way distances behave under the tensor product.. }
\end{figure}
...
\]
\caption{The magnitude of $M$ alongside the magnitudes of the factor spaces, on a logarithmic scale.}
\label{fig:prod_mag}
\end{figure}
\end{examp}
...
%TODO: Omit additivity if I don't need it.
...
\end{bmatrix}
\end{align*}
The similarity matrices of $A \times B$ and $M$ have most entries identical. We can quantify the difference between them. For any $d \in (0, \infty)$, let 
\begin{align*}
\ep_d = e^{-(1 + d)} - e^{-\sqrt{1 + d^2}}\\
E_d = \begin{bmatrix}
0 & 0 & 0 & \ep \\
0 & 0 & \ep & 0 \\
0 & \ep & 0 & 0 \\
...
\[
\mu_M = \mu_{A \times B} + \sum_{i = 1}^\infty (E_d\zeta_{A \times B}^{-1})^i.
\]
...
As we move the value of $d$ closer to 1, we expect both the error bounds and the actual error to increase. The product space and the Euclidean space will differ more as their scales become more similar. Here we see what happens when $d=1$, so our scales are identical. Both spaces move smoothly from having a single cluster to having four clusters, and there is much more significant error. In particular, $M$ now forms a square of points, and points at opposite corners have a distance equal to the diagonal of the square. On the other hand, in the product space, those points have a distance equal to twice the side of the square. This error is more significant the more similar the scales of the factor spaces. CLARIFY! %TODO: Clarify
\begin{figure}[H]
\[
